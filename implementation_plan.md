...
- [x] **Stage 4a — CBF safety filter, completed.**
  ...
  - **4a.5 — HardNet robustness study (staged interventions).** Rigorous follow-up after recognizing one run can't distinguish systematic failure from variance. Full writeups in `docs/hardnet_*.md`.
    - **Plan A (3-seed variance):** worst-case degradation is *systematic* (all seeds low-20s), but capacity exists (seed-123 ceiling = 31% = filter). Solvable. `docs/hardnet_variance_study.md`.
    - **C+B (entropy/std fix + curriculum DR):** NEGATIVE — slightly hurt (worst 24.2→21.2%). Rules out exploration instability; the std blowup was a symptom, not the cause. `docs/hardnet_intervention_CB.md`.
    - **D (auxiliary feasibility loss, λ·‖u_raw−u_safe‖², λ=0.05):** POSITIVE and decisive. Worst-case **24.2 → 33.9%** (+9.7pp), nominal unchanged (91.2%), safety improved (≤0.00% exceedance). Instrumentation confirms the mechanism: raw_safe_dist 0.66→0.08 (projection→identity, full-rank Jacobian → clean gradients). `docs/hardnet_intervention_D.md`.
