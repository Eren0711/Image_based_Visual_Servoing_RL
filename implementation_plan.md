...
- [x] **Stage 4a — CBF safety filter, completed.**
  ...
  - **HardNet was NOT used.** The plan called for a learned QP approximator (HardNet) because the literature assumes the online QP is too slow for real-time inner loops, and because some setups lack an analytical model. Neither applied here: our quadprog-based QP solves in 0.15 ms/step (well under any inference budget) and the proxy linearization in `safety/cbf_lie.py` gives us closed-form Lie derivatives, so the analytical CBF-QP is both faster and exact. HardNet would only add value if we migrated to a system without analytical dynamics or to a much larger QP.
