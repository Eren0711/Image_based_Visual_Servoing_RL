...
Follow-on to the agility ablation, which reported 0% steady-turn success from
1.0x to 3.0x interceptor agility for one policy. Here we test a guidance change
that requires NO retraining: a lead-pursuit
...
Design mirrors experiments/legacy/agility_ablation.py exactly so the two are directly
comparable:
...
Run:  python experiments/legacy/lead_pursuit_eval.py
