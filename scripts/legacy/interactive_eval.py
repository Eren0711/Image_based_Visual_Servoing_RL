"""
Historical Evaluation & Visualization Script — Stage 1
=============================================
...
def print_summary(all_data: list):
    ...
    print(f"  Envelope rate    : {100*n_envelope/n:.1f}% ({n_envelope}/{n})")
...
def main():
    ...
    parser.add_argument(
        ...
        help='Acknowledge evaluation outside the frozen canonical MVP scope.'
    )
    ...
    if canonical_run:
        ...
        if args.cbf and not args.allow_scope_override:
            raise SystemExit(
                'Frozen canonical MVP scope would be changed by: --cbf. '
    ...
    if args.model and not os.path.isabs(model_path):
