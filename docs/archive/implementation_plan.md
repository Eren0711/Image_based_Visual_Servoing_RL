...
> **HISTORICAL DEVELOPMENT LOG — NOT THE CURRENT PLAN.** The active scope and
> roadmap are defined only by `PROJECT.md` and
...
- [x] **Stage 4a — CBF safety filter, completed.**
  ...
  - **4a.4 — HardNet (in-policy differentiable CBF projection).** The plan's "CBF-HardNet" in the safe-RL sense: a differentiable projection layer (Dykstra alternating projection onto the CBF half-spaces ∩ action box, `safety/hardnet_projection.py`) embedded in a custom `ActorCriticPolicy` (`safety/hardnet_policy.py`), with the per-step CBF coefficients (A, b) appended to the observation by `CBFContextWrapper` (16→36 dim). Gradients flow *through* the projection during PPO, so the policy learns the safe-set geometry instead of being externally filtered. Trained 3M steps (~57 min, warm-start copies all 13 param tensors from the 4b policy). **Locked: 1.2M ckpt.**
    - Head-to-head (200 ep, seed=1000), HardNet vs 4b-policy+external-HOCBF vs no-safety:
      - Clean:   90.0% / 87.0% / 88.5%
      - Nominal: **92.0%** / 91.0% / 79.5%
      - Hard:    59.5% / **74.0%** / 55.5%
      - Worst:   20.5% / **31.5%** / 18.0%
    - Attitude-limit exceedance: HardNet 0.00–0.02% of steps (tightest), HOCBF filter 0.06–0.12%, no-safety 9–39% (unsafe).
    - **Initial single-run verdict was "mixed" — later OVERTURNED by the robustness study below.** The single-seed run had HardNet winning clean/nominal but losing to the external filter at hard/worst, with training destabilization (reward ~20–34, action std → 2.68). That turned out to be a gradient-conditioning artifact, not a property of in-policy projection — see 4a.5.
  - **4a.5 — HardNet robustness study (staged interventions).** Rigorous follow-up after recognizing one run can't distinguish systematic failure from variance. Full writeups in `docs/legacy/studies/hardnet_*.md`.
    - **Plan A (3-seed variance):** worst-case degradation is *systematic* (all seeds low-20s), but capacity exists (seed-123 ceiling = 31% = filter). Solvable. `docs/legacy/studies/hardnet_variance_study.md`.
    - **C+B (entropy/std fix + curriculum DR):** NEGATIVE — slightly hurt (worst 24.2→21.2%). Rules out exploration instability; the std blowup was a symptom, not the cause. `docs/legacy/studies/hardnet_intervention_CB.md`.
    - **D (auxiliary feasibility loss, λ·‖u_raw−u_safe‖², λ=0.05):** POSITIVE and decisive. Worst-case **24.2 → 33.9%** (+9.7pp), nominal unchanged (91.2%), safety improved (≤0.00% exceedance). Instrumentation confirms the mechanism: raw_safe_dist 0.66→0.08 (projection→identity, full-rank Jacobian → clean gradients). `docs/legacy/studies/hardnet_intervention_D.md`.
    - **Overturned conclusion:** with D, HardNet **beats the external HOCBF filter at every condition** (worst-case 33.9% vs 31.5% mean, 40% vs 31.5% ceiling) at nominal parity and tightest safety. The in-policy projection was never inherently weaker — it was under-trained at the extremes due to gradient degradation through the active projection.
    - **Updated deployment recommendation: HardNet-D** — best worst-case robustness, nominal parity, tightest safety, single end-to-end differentiable policy (no online QP at inference). External HOCBF filter remains a valid fallback when α must be retuned without retraining. **Locked: `logs/stages/stage4a_hardnet_d_locked/` (seed 123 @ 1.0M, 88.5% nominal / 40.5% worst).**
...
| [configs/legacy/stage3_stage4.yaml](../../configs/legacy/stage3_stage4.yaml) | Add `a_max`, `w_jerk`, update `w_effort`, add `action_rate_limit` |
...
| File | Change |
|---|---|
| [configs/legacy/stage3_stage4.yaml](../../configs/legacy/stage3_stage4.yaml) | `tau_velocity`, new `tau_yaw_rate`, `d_success`, reward block (`w_image`, `w_approach`, new `w_dist_penalty`) |
...
| [configs/legacy/stage3_stage4.yaml](../../configs/legacy/stage3_stage4.yaml) | Add mass, inertia matrix, motor params, drag coefficients |
