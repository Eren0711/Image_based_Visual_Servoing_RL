...
- Driver: `run_hardnet_seeds.sh` (sequential, full CPU each, ~55 min/seed).
- Eval: each seed's checkpoints {1.0M, 1.2M, 1.4M} on **nominal** and
  **worst** conditions, 200 episodes, eval seed=1000, via `eval_stage4b.py
  --mode hardnet`. Original seed-0 (locked 1.2M) included as reference.
